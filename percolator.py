...
class PercolationPlayer:
	# This the tree data structure is what we use to store
	#game states.
	class Tree:
...
###### MINI-MAX A-B ######
##########################
#This class uses alpha beta pruning to search through the gametree
#and find the best move for the player
	class AlphaBeta:
		...
		def alpha_beta_search(self, node):
			...
			return best_state
#this method finds the max vaules
		def max_value(self, node, alpha, beta):
			...
			for state in successors:
				...
				#print(value)
			return value
#this method finds the min vaules
...
#this method evalutes the state based on the number of
#edges, nodes and connections
		def getUtility(self, node):
			...
			for v in ally:
				edges = PercolationPlayer.getEdges(node.data, v)
				for edge in edges:
		...
		#def quickOut(self, game_tree):
		#	champ = game_tree.children[0]
			#for child in self.game_tree.children:
			#	if self.getNumVerticiesDifference > self.getNumVerticiesDifference(champ):
			#		champ = child
			#return champ


		#def getNumVerticiesDifference(self, node):
			#good = 0
			#bad = 0
			...
			#	bad += 1
			#return good - bad
		#Evaluates a graph if all the nodes are connected
		def allEdgesHueristic(self, node, total):
			if total % 2 == 0:
				...
				if node.move.color == self.active_player:
					...
					return 1000
		# Winning states we came up with
...
#this creates a tree with all the possible gamestates with a depth of 2
	def createGame_StateTree(graph, player, depth, parent, move):
		...
		for v in verts:
			...
			#print(f"deepcopy{toc - tic:0.4f} seconds")
			PercolationPlayer.Percolate(newState, PercolationPlayer.GetVertex(newState,v.index))
